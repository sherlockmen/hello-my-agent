#!/usr/bin/env node
/**
 * 04.2 把文本目标变成代码位置 | [KEEP 来自 04.1] cli.ts
 *
 * 学习目标：让单次提问和连续会话共用同一个 Agent 核心。
 * 输入：provider、模型配置、可选的 --prompt。
 * 输出：运行时显示模型与工具步骤，成功后显示回答和用量。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   --doctor --> 显示运行环境 --> 结束
 *   其他输入 --> readConfig --> createModel
 *                         +-- 有 --prompt --> agentLoop --进度--> printProgress
 *                         |                         +--最终--> printReply
 *                         +-- 无 --prompt --> startTerminal（同样显示进度）
 *   任一步失败 --> explainError --> 显示安全提示 --> exit 1
 *
 * 关键点：入口只装配事件观察者，不判断模型下一步应使用哪个工具。
 * 运行观察：--prompt 和连续会话都会先显示过程，再显示最终回答。
 */
...
/**
 * 显示当前进程的运行环境，让缺少模型配置时也能排查启动问题。
 *
 * 版本、平台、架构和启动目录都直接来自 process，输出到终端。
 * 入口在读取配置前处理 --doctor，因此这里不读取 .env，也不连接模型。
 */
