/**
 * 04.3 把代码位置变成上下文 | [CHANGED] config/load-config.ts
 *
 * 学习目标：让系统提示词要求模型根据行号分段读取文件。
 ...
 *   glob + grep + 分段 read_file --------------------------------> systemPrompt
 *
 * 关键点：提示模型按需定位和读取，read_file 必须提供 offset 和 limit；程序仍不能修改文件或执行命令。
 * 运行观察：模型可以围绕 grep 返回的行号请求一段源码，是否继续读取仍由模型决定。
 */
...
// [CHANGED 04.3] 系统提示词要求模型围绕搜索行号分段读取。
