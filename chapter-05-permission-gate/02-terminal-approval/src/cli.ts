#!/usr/bin/env node
/**
 * 05.2 在终端中完成一次审批 | [CHANGED] cli.ts
 *
 * 学习目标：根据命令行选项选择环境诊断、单次提问或连续聊天。
 * 输入：模型配置选项、--doctor 和可选的 --prompt。
 * 输出：显示环境信息或启动对话；启动失败时显示错误并设置退出码。
 * 单次提问与连续聊天都通过终端层取得审批函数，入口不自行批准工具。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   --doctor --> 显示运行环境 --> 结束
 *   其他输入 --> readConfig --> createModel
 *                         +-- 有 --prompt --> runSinglePrompt --> 审批 / 回答
 *                         +-- 无 --prompt --> startTerminal ----> 审批 / 多轮输入
 *   任一步失败 --> explainError --> 显示安全提示 --> exit 1
 *
 * 关键点：入口不自行批准工具；ui/terminal.ts 提供 ApprovalHandler，Agent Loop 决定是否执行。
 * 运行观察：--prompt 和连续会话遇到 ask 时都会显示同一类审批问题。
 */
