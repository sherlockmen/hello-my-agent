 ...
 *
 * 关键点：观察者只显示状态，ApprovalHandler 才是会影响执行的控制通道；两者明确分开。
 * 运行观察：输入 y 后工具开始；输入 n、EOF 或使用非交互模式时工具不执行。
 */
...
export async function startTerminal(model: Model): Promise<void> {
  ...
  // [CHANGED 05.2] 聊天和审批共享同一个行迭代器，避免争抢 stdin。
