/**
 * 05.3 让 Agent 记住本次运行的批准 | [KEEP 来自 04.3] tools/grep.ts
 *
 * 学习目标：从文件内容中找到位置，让模型知道接下来读哪几行。
 * 输入：query 正则表达式，以及用来选文件的 glob 模式。
 * 输出：给模型的路径、行列号和匹配行；给界面的元数据不包含源码正文。
 *
...
/**
 * 限制搜索量，并返回可以直接定位源码的文本结果。
 *
 * - 输入：模型生成的参数字符串、项目根目录和可选取消信号。
 * - 输出：`content` 给模型提供位置与匹配行；`metadata` 给界面提供不含源码正文的位置信息。
