/**
 * 05.3 让 Agent 记住本次运行的批准 | [KEEP 来自 04.3] tools/glob.ts
 *
 * 学习目标：按文件名模式找到项目文件，供模型继续搜索或读取。
 * 输入：包含 pattern 的 JSON 参数，例如查找 src 下的 TypeScript 文件。
 * 输出：给模型的路径列表，以及给界面的数量、截断标记和路径数组；不读取正文。
 *
