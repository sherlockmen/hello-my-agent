/**
 * 05.3 让 Agent 记住本次运行的批准 | [KEEP 来自 04.3] tools/read-file.ts
 *
 * 学习目标：读取搜索位置附近的一段内容，让模型据此继续处理任务。
 * 输入：path、从 1 开始的 offset，以及 1 到 400 之间的 limit。
 * 输出：给模型的带行号片段，以及给界面的实际行号和是否还能继续读取。
 * Agent Loop 已先检查权限，工具仍需检查参数、真实路径、文件类型和大小。
 *
