/**
 * 05.3 让 Agent 记住本次运行的批准 | [KEEP 来自 04.3] models/client.ts
 *
 * 学习目标：让主循环用同一种消息结构调用 OpenAI 兼容接口或 Anthropic 接口。
 * 输入：本地消息、工具定义和取消信号。
 * 输出：服务商需要的请求，以及整理后的文本、工具请求和用量。
 * 这里传递工具请求，不执行文件工具；是否允许由 Agent Loop 收到结果后检查。
 *
 ...
 * - 关键步骤：把本地消息和工具定义按所选协议发送，再把响应整理成统一字段。
