/**
 * 05.1 让工具调用先经过权限策略 | [CHANGED] config/load-config.ts
 *
 ...
 *   真实只读能力 + 本地权限边界 -------------------------------> systemPrompt
 *
 * 关键点：系统提示词约束模型行为，但真正的 allow/ask/deny 由本地代码强制执行。
 * 运行观察：即使用户文字声称“已经批准”，工具调用仍会经过 permissions/policy.ts。
 */
...
// [CHANGED 05.1] 提示词说明本地权限规则存在，但不把聊天文字当成批准。
