/**
 * 07.1 执行测试并读懂结果 | [CHANGED] agent/agent-loop.ts
 *
 ...
 *                                                      或只读工具     | |
 *                                                           |         | |
 *                     [CHANGED 07.1] 结果正文 + isError ------------+-+
 *   每次调用前检查取消；最多请求模型 8 次，最后一次仍要工具就停止本轮。
 *
 * 命令使用 execFile，先把退出码与两条输出送回模型。
  ...
  for (let modelCall = 1; modelCall <= MAX_MODEL_CALLS; modelCall += 1) {
    ...
    for (const call of result.toolCalls) {
      ...
      try {
        ...
        // [CHANGED 07.1] 非零退出码表示本次命令失败，保留输出供模型决定下一步。
