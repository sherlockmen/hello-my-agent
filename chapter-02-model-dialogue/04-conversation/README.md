...
## 问题：第二次提问时，模型不知道上一轮说过什么

上一节的一轮 Agent 执行结束后，命令进程立即退出。即使再次运行命令，程序也会重新创建空历史；第二次请求只包含第二个问题，模型并不知道第一次问答发生过。

要让同一个进程持续对话，需要分别解决三个问题：

1. **历史应该保存在哪里？** 如果每读取一行就创建一个新数组，上一轮消息会丢失；历史必须在多轮输入之间继续存在。
2. **怎样保证消息顺序？** 如果上一轮请求尚未完成就启动下一轮，两次请求可能同时修改历史，保存顺序就不再等于用户输入顺序。
3. **哪些输入不应该发送给模型？** 空行、`/exit`、EOF 和 Ctrl+C 都由终端本地处理。EOF 表示输入流已经结束，在 macOS 和 Linux 终端中通常由 Ctrl+D 产生。把这些信号当成普通问题，会制造无意义的模型请求。

因此，本节要解决的问题是：**怎样建立一个持续读取输入的终端循环，让每轮请求按顺序共享同一份进程内历史，并在本地处理空行和退出信号？** 本节历史只存在于当前进程，关闭程序后不会保存到磁盘。
...
这里有两层不同的循环：终端循环等待下一次用户输入；第 03 章扩展后的 Agent Loop 会处理同一轮中的模型和工具调用。
...
第二次提问的消息会是 `user → assistant → user`。这不是模型在后台记住了你，而是程序把已完成的问答再次放进请求。退出程序后数组消失；第 13 章会加入会话文件、会话 ID 和恢复流程。

### 第三步：理解“记忆”为什么等于重发历史

本节使用的模型接口是无状态请求。服务端处理当前请求体，不会读取本地 JavaScript 数组，也不会自动取得上一次 CLI 调用的内容。要让第二轮理解第一轮，客户端必须把已经完成的 `user` 和 `assistant` 消息再次放进请求。

因此，历史增长会直接扩大后续请求。第 `n` 轮通常携带前 `n-1` 轮的消息，输入 token、序列化数据量和模型处理时间也随之增加。本节保留完整历史是为了展示机制；第 15 章会计算上下文预算，第 16 章会压缩较早历史后继续任务。

`history` 存在 Node 进程的堆内存中。退出程序后，进程地址空间被回收，数组不会保留。第 13 章会把消息序列化为可恢复的会话记录，让新进程能够继续读取。

### 第四步：区分终端循环和 Agent Loop

终端的 `while` 处理不同用户输入：读一行，等待这一轮完成，再读下一行。Agent Loop 处理同一项任务内部的步骤：调用模型；第 03 章加入工具后，它还会在同一轮中执行工具并继续调用模型。两层循环拥有不同的继续条件，不能合并。

`readline` 把 `stdin` 字节流切成一行行字符串。异步迭代器负责按顺序交付这些行；提前取得迭代器，可以避免程序在等待模型时重新注册读取逻辑。当前实现串行处理输入，保证同一个 `history` 不会被两轮请求同时修改。

### 第五步：在终端边界处理退出与显示

`printReply()` 输出文本，单次提问也复用它。Ctrl+C 触发 `AbortController`，取消当前请求并关闭输入；`finally` 负责释放资源。程序把退出状态设置为 130，这是类 Unix 系统表示进程被 Ctrl+C 中断的常用状态。此时 Ctrl+C 会退出程序；第 08 章会把它升级为可中断当前任务并继续接收输入。
...
当前方案使用 Node 标准库和一个数组，就能清楚展示“模型记忆来自客户端重发历史”。第 13 章讨论跨进程恢复时，再增加持久化层。
...
每轮成功结果都会成为下一轮上下文，失败或取消的半轮消息不会进入历史。当前模型模块只支持 OpenAI 兼容协议；下一节保持终端和 Agent Loop 不变，在模型边界增加 Anthropic 协议适配。
