...
## 问题：所有模型处理都写在命令入口中，后续能力无处扩展

上一节已经能完成一次模型请求，但所有步骤都写在 `src/cli.ts` 中：解析命令、创建模型、组织消息、发送请求和显示结果。当前代码能运行，却还没有一个明确的“Agent 执行一轮”边界。

这会带来三个问题：

1. **哪些步骤属于 Agent 核心？** 命令入口只负责接收输入和显示结果；组织上下文、调用模型和决定本轮是否成功，应该形成一条可以独立调用的执行流程。
2. **什么时候才能修改历史？** 模型请求可能成功、失败或被取消。如果请求前就把用户消息写入正式历史，失败后会留下只有 `user`、没有 `assistant` 的半轮记录。
3. **怎样避免把多轮对话和工具调用继续堆进命令入口？** 这些能力都要围绕同一轮执行扩展。如果判断继续留在入口，每增加一种能力都会让界面代码和 Agent 决策互相缠绕。

因此，本节要解决的问题是：**怎样定义一轮稳定的 Agent 执行，让它组织本轮上下文、调用模型，并且只在完整成功后提交用户问题和模型回答？** 本节仍然只处理模型直接回答的路径，真正的工具循环留到第三章。
...
失败和取消都不会留下半轮历史。02.4 的连续会话和第 03 章的工具循环都会继续使用这条提交规则。
