...
如果把服务商判断散落在命令入口、Agent Loop 和终端中，每层都要知道两套协议；以后修复历史、取消或显示逻辑时，也要检查两个分支是否仍然一致。凭据选择同样不能模糊：选择 Anthropic 后误用 OpenAI Key，不只是请求失败，还可能把认证材料发送到错误的地址。

因此，本节真正要解决的是：**怎样在运行时选择 OpenAI 或 Anthropic，把两种不同协议转换成同一种本地模型能力，并确保每种协议只读取和发送属于自己的配置。**
