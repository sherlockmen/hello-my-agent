...
连续对话可以接着前文，但换一个话题时，我们也可能希望从头开始。本练习增加 `/reset`：输入它后，下一次请求不再带旧问答，无需退出再启动程序。

## 工作原理

02.4 的连续对话依靠终端保存 `history`，再在每轮重新发送。要开始新对话，就在终端把这份数组清空。模型配置已经在启动时读好，系统提示词也由模型模块另行加入请求，都不在这个数组里，所以清空旧问答不会把它们一起清掉。
...
如果直接把 `/reset` 交给 `agentLoop()`，模型只会收到这几个字符，本地数组并不会因此改变。所以终端要先识别这条命令，清空历史，再用 `continue` 跳过模型调用、等待下一行。
...
这就是完整的新增分支。它放在本地 `/exit` 判断之后，让退出和重置都先于模型请求处理；普通文本仍按原来的方式交给核心。
...
```ts
/**
 * 02.6 练习：输入 /reset，开始一段新对话 | [CHANGED 练习] ui/terminal.ts
 *
 ...
 * 单轮失败保留已有成功历史；/reset 清空旧问答并直接等待下一行，不会发送模型请求。
...
运行下面的练习验收，它会编译刚刚修改的 `06-errors-and-usage/src/ui/terminal.ts`，再检查 `/reset` 是否清除了 OpenAI 和 Anthropic 两种协议的历史消息：
