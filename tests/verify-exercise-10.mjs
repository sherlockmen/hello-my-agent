...
const temporary = mkdtempSync(join(tmpdir(), 'hello-my-agent-exercise-10-'));
try {
  ...
  const submitted = process.argv[2] ? resolve(process.argv[2]) : join(root, 'chapter-10-terminal-ui/exercises/ui-state.mjs');
  ...
  const answer = ownAnswer ? readFileSync(submitted, 'utf8')
    : readFileSync(join(root, 'chapter-10-terminal-ui/EXERCISES.md'), 'utf8')
      .match(/<!-- solution: exercises\/ui-state\.mjs -->\s*```js\n([\s\S]*?)\n```/)?.[1];
  assert.ok(answer, '教程必须包含完整练习答案');
  const exerciseDir = join(temporary, 'chapter-10-terminal-ui/exercises');
  mkdirSync(exerciseDir, { recursive: true });
  const script = join(exerciseDir, 'ui-state.mjs');
  ...
  console.log(ownAnswer ? '✓ 第 10 章练习文件已运行，真实工具结果、界面状态与文字累计通过'
    : '○ 尚未创建练习文件；第 10 章参考答案的真实工具结果、界面状态与文字累计通过');
