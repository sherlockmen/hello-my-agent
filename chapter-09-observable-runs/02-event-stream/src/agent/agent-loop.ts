/**
 * 09.2 按顺序接收执行事件 | [CHANGED] agent/agent-loop.ts
 *
 * 学习目标：让外层事件流按顺序送达，并准确记录“用户取消”与“事件送达故障”。
 * 输入：本轮问题、已有历史、模型、取消信号、观察者、审批函数和会话只读授权。
 * 输出：成功返回 Reply 并提交历史；本轮开始后中断则补齐工具状态、保存本地说明后抛错。
 * 状态：进入核心前已取消时不新增历史；已执行的工具操作不会随中断撤销。
 *
 * 全局主流程（本节版本）：
 *   [KEEP] CLI -> 单次问题 / 终端提问 -> [NEW] streamAgentRun -> [KEEP] runAgent
 *   runAgent -> run_start -> E；再调用 agentLoop
 ...
 *   [CHANGED] X: 错误、取消或次数用尽 -> 补齐缺失工具状态 -> 保存 turn 与本地中断说明
 *             -> 向 runAgent 抛出原因 -> F
 *   [KEEP] S: run_finish/completed -> E -> runAgent 返回 Reply
 *   [KEEP] F: 信号取消且原因不是 UserFacingError？-- 是 -> run_finish/cancelled -> E
 *                                                +-- 否 -> run_finish/error -> E
 *            -> runAgent 抛出原因 -> 事件流保存失败原因
 *   E: run_start、核心过程、run_finish -> [NEW] 带 runId / sequence 的有限队列
 *      -> [CHANGED] 终端 for await -> showRun -> renderer -> completed 记录中的 Reply
 *   普通队列已有 128 条？-- 是 -> [NEW] abort(UserFacingError) -> 核心结束并清理
 *                         +-- 否 -> 普通事件入队；run_finish 另有 1 个预留位置
 *   流读完 -> 等任务结束 -> 成功返回 / 抛出失败；消费者提前退出 -> 取消 -> 等清理
 *   [KEEP] ask -> 终端审批 handler -> 返回用户决定；审批不从观察队列获取批准
 *
 * [NEW] 表示本节新增，[CHANGED] 表示本节调整，[KEEP] 表示沿用。
 * 本节只在核心的中断说明里区分取消原因，模型、权限、工具与历史提交的执行顺序沿用前节。
 * 队列溢出会用 UserFacingError 请求停止，所以历史与 run_finish 都记录错误；普通用户取消记为取消。
 * 普通 ToolError 仍回给模型继续决策。未收齐的模型片段不提交历史，已发生的工具副作用不会回滚。
 * 异步消费把显示移出直接回调，但没有暂停上游网络；消费者过慢会触发停止，不保证任意慢速都能继续。
 * 运行观察：runId 标识一轮、sequence 表示入队顺序，事件流结束或提前退出都要等待真实任务清理。
 */
