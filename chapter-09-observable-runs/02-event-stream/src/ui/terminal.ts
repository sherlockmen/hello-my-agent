/**
 * 09.2 按顺序接收执行事件 | [CHANGED] ui/terminal.ts
 *
 * 学习目标：让终端通过异步迭代接收事件，再把事件交给已有显示器。
 * 输入：用户输入、本地命令、AgentRecord 记录流，以及 Ctrl+C / SIGINT。
 * 输出：终端正文、过程与用量；showRun 从 completed 结束事件取得 Reply。
 * 状态：history 与只读授权继续属于会话，controller 属于一轮；失败不回滚已有工具操作。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   输入 -> 本地退出 / 命令？-- 是 -> 结束 / 处理后继续读取
 *                           +-- 否 -> 创建本轮 controller 与 renderer
 *   [CHANGED] streamAgentRun -> showRun 用 for await 取记录 -> renderer.observe(event)
 *   run_finish/completed -> 暂存 Reply -> 流已读完？-- 否 -> 继续读取
 *                                                    +-- 是 -> 返回 Reply -> 显示用量
 *   流失败 / 显示失败 -> 等生成器清理 -> 显示原因 -> 连续会话继续，单次模式退出
 *   Ctrl+C -> 运行中？-- 是 -> 取消本轮并解除审批等待 -> 等清理
 *                     +-- 否 -> 关闭输入 -> exit 130
 *   审批仍单独等待用户决定；非交互或 EOF 拒绝，审批不会从观察事件中取得批准
 *
 * showRun 只负责消费记录，已有 renderer 继续负责连续文字与步骤，界面不再作为核心的直接回调。
 * 记录外壳用于标识轮次与顺序；显示器只读取 event。流正常结束却没有 completed 结果时也会报错。
 * 慢消费者可能触发事件队列的错误停止；提前退出要等待任务清理，不能只停止画面更新。
 * 运行观察：显示效果沿用，完整回答从结束记录中取得；连续会话仍在本轮清理后接收下一次任务。
 */
...
// [CHANGED 09.2] 终端通过异步迭代接收同一轮的结构化事件。
