/**
 * 09.1 让界面知道任务何时结束 | [KEEP] agent/agent-loop.ts
 *
 * 学习目标：保留模型与工具循环，让外层 runAgent 说明整轮何时开始和结束。
 * 输入：本轮问题、已有历史、模型、取消信号、观察者、审批函数和会话只读授权。
 * 输出：成功返回 Reply 并提交历史；本轮开始后中断则补齐工具状态、保存本地说明后抛错。
 * 状态：进入核心前已取消时不新增历史；已执行的工具操作不会随中断撤销。
 *
 * 全局主流程（本节版本）：
 *   [KEEP] CLI -> 单次问题 / 终端提问 -> [NEW] runAgent -> run_start -> E
 *   runAgent -> [KEEP] agentLoop -> 进入前已取消？-- 是 -> F（不增加本轮历史）
 *                                               +-- 否 -> 建立 turn -> A
 ...
 *             -> 向 runAgent 抛出原因 -> F
 *   [NEW] S: run_finish/completed -> E -> 返回 Reply -> 终端补用量
 *   [NEW] F: 信号取消且原因不是 UserFacingError？-- 是 -> run_finish/cancelled -> E
 *                                                +-- 否 -> run_finish/error -> E
 *            -> 向终端抛出原因 -> 连续会话继续 / 单次模式退出
 *   E: [CHANGED] run_start、核心过程、run_finish -> 同步观察回调 -> 终端 renderer
 *   [KEEP] ask -> 终端审批 handler -> 返回真实用户决定 -> 核心继续或生成拒绝结果
 *
 * [NEW] 表示本节新增，[CHANGED] 表示本节调整，[KEEP] 表示沿用。
 * 本文件执行代码沿用 08.3；图中的整轮通知由新增的 agent/run.ts 负责，不放进模型与工具循环。
 * 每次模型请求和工具开始前检查取消，ToolError 作为工具错误继续反馈，其他失败才结束本轮。
 * 本轮开始后中断会保存完整调用、已有结果与本地状态；未收齐的模型片段只显示，不提交历史。
 * 观察者直接抛错会被隔离，但同步回调仍会占用当前线程；审批是核心明确等待的控制依赖。
 * 已发生的文件和命令副作用不会回滚。运行观察：一轮中可有多次模型决策，整轮只通知一次开始和结束。
 */
