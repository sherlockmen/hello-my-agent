/**
 * 09.1 让界面知道任务何时结束 | [CHANGED] ui/terminal.ts
 *
 * 学习目标：终端改为调用 runAgent，在沿用显示与审批的同时收到整轮开始、结束事件。
 * 输入：用户输入、本地命令、结构化事件，以及 Ctrl+C / SIGINT。
 * 输出：终端正文、过程与用量；失败显示原因，连续会话在清理后继续读取。
 * 状态：history 与只读授权属于会话，controller 属于一轮；取消不回滚工具副作用。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   读取输入 -> EOF / /exit？-- 是 -> 清理输入 -> 结束
 *                          +-- 否 -> 本地命令？-- 是 -> 处理 -> 继续读取
 *                                              +-- 否 -> 创建本轮 controller 与 renderer
 *   [CHANGED] runAgent -> 事件交给 renderer；完成返回 Reply -> 显示用量 -> 继续读取
 *                     +-> 失败 / 取消 -> 收好显示行 -> 说明状态 -> 等待下一轮
 *   Ctrl+C -> 有 active？-- 是 -> 取消本轮、清空排队输入 -> 等本轮清理
 *                        +-- 否 -> 关闭输入 -> exit 130
 *   审批 -> 可交互？-- 否 -> deny
 *                   +-- 是 -> 显示预览 -> 读取 y/s/n；EOF 拒绝，取消则解除等待
 *
 * 本节改变的是任务入口；renderer、聊天输入、审批读取和历史所有权继续沿用。
 * run_start/run_finish 会经过显示器，teaching-trace 不重复生成正文，成功 Reply 仍从返回值得到。
 * 单次 --prompt 也使用 runAgent，但取消后退出；连续会话则在资源清理后继续。
 * 运行观察：原来的文字与审批效果保留，同时观察通道能收到整轮开始和结束。
 */
 ...
 * - 本轮执行：调用 runAgent，观察回调交给 renderer，成功时使用返回的 Reply。
 ...
 * - 执行方式：调用 runAgent，事件交给 renderer，完整 Reply 仍作为返回值接收。
