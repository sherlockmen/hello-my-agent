/**
 * 09.3 让脚本读懂执行过程 | [KEEP] agent/agent-loop.ts
 *
 * 学习目标：让 text 与 JSONL 复用同一执行链，输出约定由各自消费者负责。
 * 输入：本轮问题、已有历史、模型、取消信号、观察者、审批函数和会话只读授权。
 * 输出：成功返回 Reply 并提交历史；本轮开始后中断则补齐工具状态、保存本地说明后抛错。
 * 状态：进入核心前已取消时不新增历史；已执行的工具操作不会随中断撤销。
 *
 * 全局主流程（本节版本）：
 *   [CHANGED] CLI --output -> text -> 终端提问或单次问题 --+
 *                          +-> jsonl + --prompt ----------+-> [KEEP] streamAgentRun
 *   streamAgentRun -> [KEEP] runAgent -> run_start -> E；再调用 agentLoop
 ...
 *   [KEEP] X: 错误、取消或次数用尽 -> 补齐缺失工具状态 -> 保存 turn 与本地中断说明
 *             -> 向 runAgent 抛出原因 -> F
 ...
 *      -> 消费者？-- text -> [CHANGED] 正文 stdout，过程 / 审批 / 用量 stderr
 *                 +-- jsonl -> [NEW] 一条 JSON + 换行 -> 等写入回调 -> 下一条
 *   [KEEP] 队列超限 / 消费者提前退出 -> 取消同一轮 -> 等模型与工具清理
 *   [NEW] JSONL stdout 出错 -> abort(UserFacingError) -> 等清理 -> stderr、exit 1
 *   [NEW] JSONL SIGINT -> abort -> 等清理 -> stderr、exit 130
 *   ask -> text 可交互？-- 是 -> 等用户决定；否 -> deny
 *       +-> [NEW] jsonl -> 直接 deny；两种结果都回到同一个核心
 *
 * [NEW] 表示本节新增，[CHANGED] 表示本节调整，[KEEP] 表示沿用。
 * 本文件执行代码沿用 09.2；新增的是 ui/jsonl.ts 与入口、文本输出位置，核心不负责 JSON 编码。
 * 普通事件最多暂存 128 条，结束事件另有一个位置；这是事件数量限制，不是总字节内存硬上限。
 * 历史中的错误与取消继续区分，已发生的文件和命令副作用不回滚；取消等待与资源清理由同一链条处理。
 * JSONL 保留原始事件内容，没有完整脱敏；输出管道已关闭时，也不能保证再送出一条结束记录。
 * 运行观察：同一任务可给终端显示，也可逐行交给脚本解析；JSONL 不读取审批输入。
 */
