/**
 * 09.3 让脚本读懂执行过程 | [CHANGED] ui/terminal.ts
 *
 ...
 * 状态：本节只改变输出位置，历史、只读授权、异步消费和取消清理继续沿用。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   事件流 -> showRun -> renderer -> 模型文字？-- 是 -> stdout
 *                                            +-- 否 -> [CHANGED] 步骤说明写 stderr
 *   completed -> Reply -> 正文已显示？-- 否 -> stdout 补正文
 *                                    +-- 是 -> 不重复正文
 *                         -> [CHANGED] 用量与截断提示写 stderr
 *   审批 -> [CHANGED] stderr 显示请求与预览 -> 可交互？-- 是 -> 共用输入读取决定
 *                                                             +-- 否 -> deny
 *   聊天 -> 本地命令 / 中断说明 -> [CHANGED] stderr；退出时仍等待本轮与输入清理
 *
...
// [CHANGED 09.3] 审批提示写入 stderr，不混入模型正文。
...
// [CHANGED 09.3] readline 的回显与审批提示使用 stderr。
