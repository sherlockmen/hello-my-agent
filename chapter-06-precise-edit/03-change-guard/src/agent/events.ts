/**
 * 06.3 保存之前，检查文件有没有变化 | [KEEP 来自 06.2] agent/events.ts
 *
...
export type AgentEvent =
  ...
  // [KEEP 来自 06.1] 成功事件只额外保存 previewChars，不复制完整 diff；原始 call 仍可能包含完整参数。
  ...
  // [KEEP 来自 06.1] 观察者只知道是否有预览；完整 diff 由审批适配器负责展示。
  | {
      type: "approval_start";
      sequence: number;
      call: ToolCall;
