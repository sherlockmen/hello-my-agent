/**
 * 06.3 保存之前，检查文件有没有变化 | [KEEP 来自 06.2] ui/terminal.ts
 *
 * 学习目标：把完整差异展示给用户，再把用户的决定交回主循环。
 * 输入：聊天文字、本地命令，以及审批时输入的 y / s / n；它们共用一个行迭代器。
 * 输出：写入先显示 diff，只接受本次允许或拒绝；读取仍可按策略选择会话允许。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   读取一行 -> 本地命令？-- 是 --> 执行命令 -> 继续读取，或退出
 *                         +-- 否 --> 调用 agentLoop -> 显示回答或错误 -> 继续读取
 *   审批请求 -> 可交互？-- 否 --> 返回拒绝
 *                       +-- 是 --> 显示请求和可选 diff -> 读取决定 -> 返回核心
 *   Ctrl+C -> 取消请求并关闭输入 -> exit 130
 *
 * 终端只收集决定，是否执行仍由主循环控制。没有输入通道或遇到 EOF 时默认拒绝。
 * 运行观察：write_file 先显示新内容，输入 y 后才有创建记录。
 * /reset 清空的是对话历史，不会恢复已经改过的文件，也不会撤销已有只读授权。
 */
 ...
 * - 有 preview 就先打印完整 diff；能复用的读取可选 y / s / n，写入只接受 y / n。
...
export function createApprovalHandler(
  ...
  return async (request, signal) => {
    ...
    // [KEEP 来自 06.1] 审批界面显示由待写入内容计算出的完整差异，不接受模型的文字摘要代替。
