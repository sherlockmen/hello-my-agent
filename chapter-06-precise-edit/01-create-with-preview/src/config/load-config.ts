/**
 * 06.1 先预览，再创建文件 | [CHANGED] config/load-config.ts
 *
 * 学习目标：读出模型连接配置，并准确告诉模型当前 Agent 能做什么。
 * 输入：provider、model、baseUrl 选项和对应的环境配置。
 * 输出：校验后的 Config，以及说明读取和创建文件能力的 systemPrompt。
 *
...
// [CHANGED 06.1] 只声明已接通的创建能力：write_file 先展示 diff，只创建不存在的文件；
// 提示词引导模型选择工具，本地权限和 wx 才强制执行边界。
