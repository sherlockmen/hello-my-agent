/**
 * 06.2 找到原文，只替换这一处 | [CHANGED] tools/registry.ts
 *
...
// [CHANGED 06.2] edit_file 与 write_file 共用准备、审批和执行入口。
 ...
 * 找到本次写入工具，先让它准备内容和预览。
 *
 * write_file 和 edit_file 返回 PreparedToolCall，只读工具返回 null。
...
export async function prepareTool(
  ...
  // [CHANGED 06.2] edit_file 复用既有准备接口；Agent Loop 不新增按工具名分支。
