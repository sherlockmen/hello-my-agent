...
命令在两个终端里表现不同时，我们先想知道它们使用的 Node、运行平台和工作目录有没有区别。本练习增加 `--doctor`，把这些信息直接显示出来。

## 工作原理

`--doctor` 做的事很简单：Commander 识别这个选项后，程序读取 Node 提供的进程信息，打印出来，然后结束本次操作。它帮助我们比较运行条件，还不会判断版本是否合适，也不检查模型服务。
...
这个选项要先用 `.option("--doctor")` 登记，才能在 `.action()` 中使用。Commander 只会解析已经登记的规则；未登记的选项会在进入 `.action()` 前被当成未知输入。另一个常见错误是显示诊断后忘记 `return`，导致程序继续输出默认欢迎语。
...
以上输出仅作示例。实际结果取决于当前使用的 Node 版本、系统、架构和工作目录；`darwin` 是 Node 对 macOS 的平台标识。使用 `--doctor` 时应只显示这三行，不再显示欢迎语。
...
完成练习后，在项目根目录执行 `npm run lesson:02.1` 继续学习第二章。第二章的命令入口会保留 `--doctor`，并在它之后增加模型配置、请求和会话能力。
