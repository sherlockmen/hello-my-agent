...
这仍然只是一组来自模型的数据。直接把 `arguments` 传给文件 API 会留下四个问题：参数可能不是有效 JSON，路径可能离开当前项目，模型可能请求包含密钥的 `.env`，结果返回后模型也不知道它对应哪一次请求。

此外，一次工具执行不是一轮 Agent 的终点。模型拿到文件内容后，还要再次推理并产生最终回答。如果 `agentLoop()` 仍只调用模型一次，工具结果永远不会影响回答。

所以，本节要解决的问题是：**怎样安全地执行一次真实文件读取，把结果加入正确的消息位置，并继续调用模型直到得到最终回答？**
