/**
 * 03.1 识别模型的工具请求 | [KEEP 来自 02.6 和第二章练习] ui/terminal.ts
 *
 * 学习目标：持续读取终端输入，并让整个进程中的多轮对话共享同一个 history。
 * 输入：逐行用户文本、/reset、/exit、EOF 或 Ctrl+C。
 * 输出：普通文本交给 agentLoop()；/reset 清空历史；退出信号关闭输入和请求。
 *
 ...
 *
 * 关键点：history 在 while 外创建，所以第二轮能带上第一轮问答；/reset 把数组长度设为 0，下一轮不再携带旧消息。成功时显示 token 用量与截断提示；失败时通过 explainError() 给出分类建议。
 * 交互终端中“你”为青色、“Agent”为紫色；管道和文件输出不包含 ANSI 控制字符。
 * 这个 while 等待不同的用户输入。本节的 agentLoop 只能识别工具请求并停止；
 * 03.2 才会在 agent/agent-loop.ts 中加入执行工具、回传结果和再次请求模型的循环。
