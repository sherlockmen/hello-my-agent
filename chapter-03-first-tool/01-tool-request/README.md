...
**本节目标：向模型声明 `read_file`，把两种接口返回的工具字段统一成 `ToolCall`，但暂不读取磁盘。**

## 问题

第二章的模型客户端只接受纯文本回答。它遇到工具请求时会报错，因为工具响应通常没有普通正文：
...
如果程序只检查 `content`，就会把一次合法的工具请求误判成“空回答”。如果看到 `name: "read_file"` 就直接调用同名 JavaScript 函数，又会把模型输出当成可信指令，跳过本地允许列表和参数校验。

程序需要先回答两个问题：这次响应是最终回答还是工具请求？不同服务商返回的字段怎样交给同一个 Agent 核心？

所以，本节要解决的问题是：**怎样让模型用结构化数据提出 `read_file` 请求，并把两种协议转换成统一、尚未执行的工具请求？**

## 解决方案

先定义工具的公开契约，再由模型适配层发送契约并归一化响应。Agent Loop 只识别分支；发现工具请求后明确停止，证明本节尚未访问文件。
...
这个检查放在模型适配边界，而不是文件工具内部。这样无论后面增加多少工具，无效的服务商响应都不会进入 Agent Loop，更不会开始本地执行。
...
| 文件 | 状态 | 职责 |
| --- | --- | --- |
| `src/tools/read-file.ts` | 新增 | 定义 `read_file` 的名称、说明和参数 Schema |
| `src/tools/registry.ts` | 新增 | 汇总工具定义，声明统一 `ToolCall` |
| `src/models/client.ts` | 修改 | 发送工具定义，归一化两种协议的工具请求 |
| `src/agent/agent-loop.ts` | 修改 | 区分最终回答与工具请求 |
| `src/config/load-config.ts` | 修改 | 系统提示词说明当前只能提出 `read_file` 请求，尚不能取得结果 |
...
- `readFileDefinition` 描述工具名称、用途和参数。
- `ToolCall` 保存调用 ID、名称和原始 JSON 参数。
- OpenAI 与 Anthropic 的工具请求转换成同一结构。
- `ModelResult` 同时容纳文本和工具请求。
- Agent Loop 能识别工具分支，但不会提前访问磁盘。
- 系统提示词只声明真实存在的能力。
...
现在程序能听懂“请调用 `read_file`”，却仍停在请求数据上。[03.2](../02-read-file-loop/README.md) 将校验参数和真实路径，执行文件读取，把结果按调用 ID 发回模型，并让 Agent Loop 继续到最终回答。
