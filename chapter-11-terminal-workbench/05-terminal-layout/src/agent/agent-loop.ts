/**
 * 11.5 按终端列宽排版 | [KEEP 来自 10.4] agent/agent-loop.ts
 *
 * 学习目标：窗口缩放只重新排版画面，草稿、历史与执行状态继续保留。
 ...
 *                                                    +-- 是 -> 确定输出模式
 *   [KEEP] auto：无 --prompt 且为可交互终端 -> tui；其余 -> text
 *   读取配置、创建模型 -> 选择消费者
 *   消费者 -> text -> 单次问题或逐行输入 -------------------------------> I
 *              +-> [KEEP] tui -> 终端条件满足且无 --prompt？-- 否 -> 错误结束
 *              |                                          +-- 是 -> startTui -> W
 *              +-> [KEEP] jsonl + --prompt ---------------------------------> R
 *   W: [KEEP] 草稿键入 / 换行 / 粘贴 / 撤销 -> 更新 Draft -> 留在 W
 *      [KEEP] 找回 / 搜索用户问题 -> 填回 Draft -> W
 *      [KEEP] Tab -> 命令或 @路径候选 -> 更新 Draft / 显示提示 -> W
 *      [KEEP] Ctrl+O -> 草稿 / 历史焦点；历史操作 -> 滚动 / 折叠 / 复制 -> W
 *      [KEEP] 审批出现 -> H 独占输入；审批结束 -> 恢复原草稿与浏览状态
 *      [KEEP] Ctrl+G -> 空闲且无审批？-- 否 -> 提示稍后编辑 -> W
 *                                         +-- 是 -> Ink 让出终端 -> 外部编辑器
 *         编辑成功且内容有效？-- 否 -> 保留原草稿、恢复终端 -> W
 *                              +-- 是 -> 更新草稿、恢复终端 -> W（仍未发送）
 *      [NEW] 窗口不足 40 列 / 16 行 -> 保留状态、停用普通输入 -> 等放大
 *      [NEW] 可用窗口 -> 字素与列宽排版；正文偏移重新定位历史 -> W
 *      普通 Enter -> 本轮仍在运行？-- 是 -> 保留草稿，提示等待 -> W
 *                                    +-- 否 -> 完整草稿 -> I
 *   I: 本地命令？-- 是 -> 清理历史 / 查看授权 / 撤销只读授权 -> 返回输入区
 *                |        /exit -> 退出 Q
 *                +-- 否 -> 空白？-- 是 -> 返回输入区；否 -> R
 ...
 *                 +-- tui -> [KEEP] 事件 -> RunView（含完整工具显示正文）-> 稳定 ID 的条目 -> 历史视口
 *   H: text 可交互？-- 是 -> 等用户决定；否 -> deny
 *      jsonl -> [KEEP] 直接 deny；tui -> [KEEP] 审批面板 -> Promise 等 y / n / s -> 返回原审批调用
 ...
 *   [KEEP] TUI Ctrl+C -> 本轮在运行？-- 是 -> abort -> 等清理 -> 回到 W
 *                                                +-- 否 -> 退出 Q
 *   [KEEP] text 连续会话 Ctrl+C -> 有任务？-- 是 -> abort -> 等清理 -> I；否 -> 退出 Q
 *   [KEEP] 外部编辑期间的 Ctrl+C 交给编辑器；界面不同时退出
 *   Q: 交互退出 -> 请求取消 -> 等模型任务与外部编辑结束 -> 清理消费者资源
 *      TUI 卸载界面并移除监听；text 关闭输入读取器与终端 -> 返回 CLI
 *   消费结束且仍在交互会话？-- 是 -> tui 回 W，text 回 I；否 -> 返回 CLI 并结束
 *
 * [NEW] 表示本节新增，[CHANGED] 表示本节调整，[KEEP] 表示沿用。
 * 本文件执行代码沿用 10.4；本节增量位于 ui/tui/，图中新增节点不改变模型和工具循环。
 * 任务未结束时可继续编辑草稿，但不能发送下一轮；下一轮使用新控制器、原历史和只读授权。
 * 普通事件最多暂存 128 条，结束事件另有一个位置；这是条数限制，不是总字节内存硬上限。
 * JSONL 没有完整脱敏；TUI 处理的是显示副本，不改变发给模型的正文、权限范围或工具结果。
 * 运行观察：窗口缩放后仍可找到原先浏览的正文，模型与工具任务照常按事件推进。
 */
