/**
 * 11.4 把草稿交给外部编辑器 | [KEEP 来自 10.4] agent/agent-loop.ts
 *
 * 学习目标：空闲时把草稿交给外部编辑器，恢复终端后仍由用户决定发送。
 ...
 *   [NEW] 外部编辑期间的 Ctrl+C 交给编辑器；界面不同时退出
 ...
 * 运行观察：编辑器退出后回到草稿区，只有再次发送才进入 Agent Loop。
 */
