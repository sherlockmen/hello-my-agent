/**
 * 11.2 找回旧问题，补全当前输入 | [KEEP 来自 10.4] agent/agent-loop.ts
 *
 * 学习目标：让问题历史和补全先回到草稿，用户确认发送后再进入原执行链。
 ...
 *      [NEW] 找回 / 搜索用户问题 -> 填回 Draft -> W
 *      [NEW] Tab -> 命令或 @路径候选 -> 更新 Draft / 显示提示 -> W
 *      普通 Enter -> 本轮正在运行？-- 是 -> 普通编辑器未激活 -> 等结束
 *                                    +-- 否 -> 完整草稿 -> I
