/**
 * 08.2 收齐工具参数，再执行 | [CHANGED] models/client.ts
 *
 ...
 *
 * [CHANGED] 表示本节修改。SDK 的流中途对象可能来自部分 JSON 解析，不能证明参数已经完整。
 ...
 */
// [CHANGED 08.2] 片段用于显示，完整响应才进入统一结果。
async function requestResult(
  ...
  if (client instanceof OpenAI) {
    ...
    // [CHANGED 08.2] 结束原因正常才接收工具；拒绝和截断只交给核心说明。
  ...
  // [CHANGED 08.2] Anthropic 的文字和工具参数分开接收；SDK 负责 SSE 与完整消息聚合。
