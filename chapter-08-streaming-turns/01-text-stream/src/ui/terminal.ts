/**
 * 08.1 让回答逐段显示 | [CHANGED] ui/terminal.ts
 *
 * 学习目标：把一个回答逐段接到同一处，并在步骤记录和错误出现前收好显示行。
 * 输入：用户文字、本地命令、审批决定，以及 Agent Loop 的文字和步骤事件。
 * 输出：连续文字、工具过程与本轮用量；显示状态由每轮独立的 renderer 保存。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   [KEEP] 读取一行 -> 本地命令？-- 是 -> 处理后继续输入，或退出
 *                                +-- 否 -> [NEW] 创建本轮 renderer -> 等待本轮结果
 *   renderer 收到事件 -> text_delta？-- 是 -> 追加文字，记为已显示
 *                                    +-- 否 -> 补换行 -> model_start？-- 是 -> 清空显示标记
 *                                                                     +-- 否 -> 保留标记
 *   model_finish -> 本次没有片段且有完整文字？-- 是 -> 显示正文并标记
 *                                             +-- 否 -> 只显示步骤
 *   本轮完成 -> 正文显示过？-- 是 -> 只补用量 / 否 -> 显示正文与用量
 *   本轮失败 -> 补换行 -> 显示错误；Ctrl+C 则取消请求、关闭输入并退出
 *
 * [NEW] 是本节新增，[KEEP] 是沿用；聊天与审批仍共用一个行迭代器。
 * 同一轮可能多次请求模型，所以“文字是否显示过”在 model_start 时重置。
 * 屏幕上的片段不等于已提交历史；本文件只显示，提交规则仍在 Agent Loop。
 * 本节 Ctrl+C 仍退出程序；08.3 再改为运行中只取消当前任务。
 * 运行观察：OpenAI 回答逐段出现，结束时不会把整段回答打印第二遍。
 */
 ...
 */
// [CHANGED 08.1] 本轮的进度、回答和错误共用同一显示器。
 ...
 */
// [CHANGED 08.1] 单次提问也使用本轮显示器，结束时只补尚未显示的部分。
