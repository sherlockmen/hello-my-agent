/**
 * 08.1 让回答逐段显示 | [CHANGED] ui/teaching-trace.ts
 *
 * 学习目标：让模型、权限和工具步骤保持简短，连续文字交给专门的显示器。
 * 输入：Agent Loop 的结构化事件；输出：供终端打印的纯文本行数组。
 *
 * 本文件局部流程（全局主流程见 agent/agent-loop.ts）：
 *   [CHANGED] 事件 -> text_delta？-- 是 -> 空数组，由 renderer 追加正文
 *                              +-- 否 -> 根据事件类型选字段
 *                                     -> 清理控制字符、隐藏常见凭据并限长 -> 文本行
 *
 * [CHANGED] 表示本节调整。本节新增 text_delta 的空数组分支，避免每段文字都变成一条步骤日志。
...
export function formatTeachingTrace(event: AgentEvent): string[] {
  // [NEW 08.1] 连续文字由终端追加，不能每段都包装成一行日志。
