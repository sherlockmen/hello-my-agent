/**
 * 10.1 把对话放进一个界面 | [KEEP] agent/agent-loop.ts
 *
 * 学习目标：让新界面接上原有执行链，先显示完整回答，再继续等待输入。
 ...
 *              +-> [NEW] tui -> 终端条件满足且无 --prompt？-- 否 -> 错误结束
 *              |                                          +-- 是 -> startTui -> I
 *              +-> [KEEP] jsonl + --prompt ---------------------------------> R
 ...
 *                 +-- tui -> [NEW] 消费整轮事件，仅在 completed 时显示完整 Reply
 *   H: text 可交互？-- 是 -> 等用户决定；否 -> deny
 *      jsonl -> [KEEP] 直接 deny；tui -> [KEEP] 尚未传入审批函数 -> deny
 ...
 *   [NEW] TUI Ctrl+C -> abort -> 等本轮清理 -> 退出 Q
