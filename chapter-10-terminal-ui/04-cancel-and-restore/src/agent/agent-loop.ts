/**
 * 10.4 取消以后，继续留在对话里 | [KEEP] agent/agent-loop.ts
 *
 * 学习目标：取消当前任务后等待清理，再用保留下来的会话接收下一个问题。
 ...
 *                                                    +-- 是 -> 确定输出模式
 *   [CHANGED] auto：无 --prompt 且为可交互终端 -> tui；其余 -> text
 *   读取配置、创建模型 -> 选择消费者
 ...
 *   [CHANGED] TUI Ctrl+C -> 本轮在运行？-- 是 -> abort -> 等清理 -> 恢复输入 I
 *                                                +-- 否 -> 退出 Q
